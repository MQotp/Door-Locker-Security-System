 ...
 * [Function Description]: Function to make delay for certain time based on the processor speed.
 *
 *
 * Inputs: Number of milliseconds to delay with.
 * Outputs: No Outputs.
 */
/*void msDelay(uint16 milliseconds)
{
	uint32 i,counts;
	counts = ((F_CPU * milliseconds)/1000);
	for(i = 0; i < counts; i++);
}*/
